...
class JSSDK {
  ...
  private function getJsApiTicket() {
    ...
    $data = json_decode(file_get_contents("../public/runtime/wap/jsapi_ticket.json"));
    if ($data->expire_time < time()) {
      ...
      if ($ticket) {
        ...
        $fp = fopen("../public/runtime/wap/jsapi_ticket.json", "w");
  ...
  private function getAccessToken() {
    ...
    $data = json_decode(file_get_contents("../public/runtime/wap/access_token.json"));
    if ($data->expire_time < time()) {
      ...
      if ($access_token) {
        ...
        $fp = fopen("../public/runtime/wap/access_token.json", "w");
        fwrite($fp, json_encode($data));
        fclose($fp);
      }
    } else {
