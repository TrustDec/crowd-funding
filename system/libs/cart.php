			...
			if($GLOBALS['db']->affected_rows()>0)
			{
				...
				if($order_info)
				{
					...
					else
					{
						...
						if($result['status']==3)
						{
							return array("info"=>"<a href='".url("deal#show",array("id"=>$order_info['deal_id']))."'>".$order_info['deal_name']."</a>支付成功","jump"=>url_get_account($order_id)); 
						}
						if($result['status']==0)
						{
							return array("info"=>"<a href='".url("deal#show",array("id"=>$order_info['deal_id']))."'>".$order_info['deal_name']."</a>订单被篡改，支付失败","jump"=>url_get_account($order_id)); 
						}
					}
				}
				else
