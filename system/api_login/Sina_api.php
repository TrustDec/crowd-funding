...
class Sina_api implements api_login {
	...
	public function callback()
	{
		...
		$user_data = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."user where sina_id = '".$api_data['id']."' and sina_id <> ''");	
		...
		{			
			if($GLOBALS['user_info'])
			{
				...
				$GLOBALS['db']->query("update ".DB_PREFIX."user set sina_id = '".$api_data['id']."',sina_token = '".$api_data['token']."',sina_url = '".$api_data['url']."' where id =".intval($GLOBALS['user_info']['id']));								
	...
	public function unset_api(){
	    if($GLOBALS['user_info']){
	       $GLOBALS['db']->query("update ".DB_PREFIX."user set sina_id= '', sina_token ='',sina_secret='',sina_url='' where id =".$GLOBALS['user_info']['id']);
	    }
	}   
