...
KE.event = {
	...
	input : function(el, func, id) {
		...
		this.add(newElement, 'paste', pasteHandler, id);
...
KE.plugin.emoticons = {
	...
	exec : function(id, num) {
		...
		var kesrc = './public/emoticons/' + num + '.gif';
		var html = '<img src="' + src + '" kesrc="' + kesrc + '" alt="" />';
		KE.util.insertHtml(id, html);
		this.menu.hide();
		KE.util.focus(id);
	}
...
KE.plugin.image = {
	...
	insert : function(id, url, title, width, height, border, align) {	
		re = /[.]\/public\//g;  
		var realsrc = url.replace(re,APP_ROOT+"/public/");
		var html = '<img src="' + realsrc + '" kesrc="' + url + '" ';
