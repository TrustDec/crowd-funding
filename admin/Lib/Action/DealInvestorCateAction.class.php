...
class DealInvestorCateAction extends CommonAction{
	...
	public function insert() {
		...
		if (false !== $list) {
			clear_auto_cache("deal_nav_cate",array('name'=>'deal_investor_cate'));
			//成功提示
	...
	public function set_effect()
	{
		...
		if($c_is_effect ==1)
		{
			...
			$deal_count=D("Deal")->where("cate_id in (".$cate_ids_str.") and type=1")->count();
			if($deal_count >0)
			{
				$this->error("该分类或子类，股权众筹项目在使用，不能删除",1);
			}
		}
