...
(function($){
	$.fn.jcDate = function(options) {
		var defaults = {
			...
			format : "/",
			Timeout : 100,
			Oldyearall : 9,  // 配置过去多少年
			Newyearall : 8  // 配置未来多少年
		};
		...
		return this.each(function() {
			...
			var NowY = _year,
				yArr = new Array(),
				mArr = new Array();
			for(var lys = 0; lys < options.Oldyearall ; lys++ ){
				lastyear = NowY - (lys+1);
			...
			var dtf = ysArr(yArr);
            yArr.length = 0;
			for(var fys = 0; fys < options.Newyearall ; fys++ ){
			...
			$dateInput.addClass(options.IcoClass)
					  ...
					  .live(options.Event,function(e){
						  ...
						  var iof = $(this).offset();
						  $jcDate.css({ "left" : iof.left+options.Left+125,"top" : iof.top+options.Top+13 });
