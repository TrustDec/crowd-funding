...
require './app/Lib/app_init.php';
		...
		$name=strim($_REQUEST['key']);
		file_put_contents("ts.txt", print_r($name,1));
		//上传处理
		//创建comment目录
		...
		if(app_conf("IS_WATER_MARK")==1)
			$img_result = save_image_upload($_FILES,$name,"attachment/".$dir,$whs=array('thumb'=>array(205,160,1,0)),1,1);
		else
			$img_result = save_image_upload($_FILES,$name,"attachment/".$dir,$whs=array('thumb'=>array(205,160,1,0)),0,1);
		...
		$file_url = $img_result[$name]['url'];
		$thumb_url = $img_result[$name]['thumb']['thumb']['url'];
