...
class collocationModule
{
	...
	public function CreateNewAcct(){
		...
		if (empty($data)){
			echo "用户不存在!<br />请点左上角<b>返回</b>按钮";
			exit;
		}else{		
			if (empty($data['ips_acct_no'])){					
				if (empty($data['identify_number'])){
					echo "身份证号码不能为空,请补充完资料后再申请<br />请点左上角<b>返回</b>按钮";
					exit;
				}else if (empty($data['identify_name'])){
					echo "真实姓名不能为空,请补充完资料后再申请<br />请点左上角<b>返回</b>按钮";
					exit;
				}else if (empty($data['mobile'])){
					echo "手机号码不能为空,请补充完资料后再申请<br />请点左上角<b>返回</b>按钮";
					exit;
				}
				// else if (empty($data['email'])){
					// echo "邮箱不能为空,请补充完资料后再申请<br />请点左上角<b>返回</b>按钮";
					// exit;
				// }
				else{	
				...
				echo "该用户已经申请过资金托管帐户:<br />请点左上角<b>返回</b>按钮";
				exit;
			}
		}
	}
	...
	public function DoDpTrade(){
		...
		if (empty($data)){
			...
			if (!empty($data['ips_acct_no'])){
				...
				echo "该用户还未申请过资金托管帐户!<br />请点左上角<b>返回</b>按钮";
				exit;
			}
		}
	}
	...
	public function DoDwTrade(){
		...
		//判断是否是黑名单会员
    	if($$data['is_black']==1){
    		echo "您当前无权限提现，具体联系网站客服<br />请点左上角<b>返回</b>按钮";
    		exit;
    	}
	...
	public function RegisterCreditor(){
		...
		$order_id = intval($_REQUEST['order_id']);
 		...
 		$order_user = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."user where id = ".$user_id);
 		
 		if($sign!=md5($order_user['paypassword'].$order_id)){
			echo "支付密码错误!<br />请点左上角<b>返回</b>按钮";
			exit;
		}
		
 		$order_info=$GLOBALS['db']->getRow("select * from ".DB_PREFIX."deal_order where user_id=".$user_id." and id=$order_id");
 		if(!$order_info){
 			echo "订单不存在!<br />请点左上角<b>返回</b>按钮";
 			exit;
 		}
		...
		if (!empty($deal)){
			if (!empty($deal['ips_bill_no'])){
				...
				if (!empty($user) && !empty($user['ips_acct_no'])){
					...
					if(empty($user)){
						echo "项目发起人不存在!<br />请点左上角<b>返回</b>按钮";
 						exit;
					}else{
						echo "项目发起人未申请资金托管帐户!<br />请点左上角<b>返回</b>按钮";
 						exit;
					}
				}
				
			}else{
				echo "该产品并非第三方托管支付!<br />请点左上角<b>返回</b>按钮";
 				exit;
			}
		}else{
			echo "该产品不存在:".$order_info['deal_id']."!<br />请点左上角<b>返回</b>按钮";
 			exit;
		}
	 }	
