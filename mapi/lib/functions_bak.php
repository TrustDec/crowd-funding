...
function formateType2DealInfo($deal_info)
{
	...
	$deal_info_type2 ['invote_money'] = floatval ( $deal_info ['invote_money'] );
...
/**
 * 股权deal Status判断 0即将开始 1已成功 2筹资失败 3长期项目 4融资中
 *          deal_status 1待审核 2预热中3 进行中 4认投成功 5融资成功
 */
function formateDealInfoType2Status($deal_info_type2, $deal_info)
{
	...
	}else if($deal_info['begin_time']<NOW_TIME&&$deal_info['end_time']>NOW_TIME&&$deal_info['is_success']==1)
	{
		$deal_info_type2['deal_status']=4;
		$deal_info_type2['deal_status_expression']='认投成功';
	}else if($deal_info['end_time']<NOW_TIME&&$deal_info['is_success']==1&&$deal_info['invest_status']==1)
	{
		$deal_info_type2['deal_status']=5;
		$deal_info_type2['deal_status_expression']='融资成功';
	}
...
/**
 *处理普通众筹的状态等
 * status值0表示即将开始；1表示已成功；2表示筹资失败；3表示筹资中；4表示长期项目 剩余时间remain_days
 * deal_status 1待审核 2进行中 3 已成功 4 已失败 5 未通过
 */
function handleDealType0Status($deal_info,$deal_info_type1)
{
	...
	if ($deal_info ['begin_time'] > $now_time)
	{
		...
		$deal_info_type1 ['status_expression'] = '即将开始';
	} elseif ($deal_info ['end_time'] < $now_time && $deal_info ['end_time'] > 0)
	{
	...
	}else if($deal_info['begin_time']<NOW_TIME&&$deal_info['end_time']>NOW_TIME )
	{
		$deal_info_type1['deal_status']=2;
		$deal_info_type1['deal_status_expression']='进行中';
	}else if($deal_info['is_success']==1&&$deal_info['end_time']<NOW_TIME)
	{
