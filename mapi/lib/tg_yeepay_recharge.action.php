...
class tg_yeepay_recharge{
	...
	{		
		...
		$yeepay_recharge_count = $GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."yeepay_recharge where platformUserNo = ".$user_id."  ");
		if( $yeepay_recharge_count >0 )
		{
			$yeepay_recharge_list = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."yeepay_recharge where platformUserNo = ".$user_id." order by create_time desc,id desc limit ".$limit);
			$return_list=array();
			foreach($yeepay_recharge_list as $k=>$v)
			{
				...
				if($v['code'] =='1')
				{
					...
					$return_list[$k]['code_info'] = '已支付';
				}
				else
				{
					$return_list[$k]['code'] = 0;
					$return_list[$k]['code_info'] = '未支付';
				}
			}
		}else{
			$return_list=array();
