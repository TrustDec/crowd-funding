		...
		if (empty($data)){
			...
			if (empty($data['ips_acct_no'])){					
				if (empty($data['identify_number'])){
					showErr('身份证号码不能为空,请补充完资料后再申请',0,url_wap("settings#security",array('method'=>'setting-mobile-box')));
				}else if (empty($data['identify_name'])){
					showErr('真实姓名不能为空,请补充完资料后再申请',0,url_wap("settings#security",array('method'=>'setting-mobile-box')));
				}else if (empty($data['mobile'])){
					showErr('手机号码不能为空,请补充完资料后再申请',0,url_wap("settings#security",array('method'=>'setting-mobile-box')));
				}
				// else if (empty($data['email'])){
					// showErr('邮箱不能为空,请补充完资料后再申请',0,url_wap("settings#security",array('method'=>'setting-email-box')));
				// }
				else{
					if($data['is_investor']==2){
						if (empty($data['identify_business_name'])){
							showErr('企业名称不能为空,请补充完资料后再申请',0,url_wap("user#register_two"));
						}elseif(empty($data['bankLicense'])){
							showErr('开户许可证不能为空,请补充完资料后再申请',0,url_wap("user#register_two"));
						}elseif(empty($data['orgNo'])){
							showErr('组织机构代码不能为空,请补充完资料后再申请',0,url_wap("user#register_two"));
						}elseif(empty($data['taxNo'])){
							showErr('税务登记号不能为空,请补充完资料后再申请',0,url_wap("user#register_two"));
						}elseif(empty($data['businessLicense'])){
							showErr('营业执照编号不能为空,请补充完资料后再申请',0,url_wap("user#register_two"));
						}elseif(empty($data['contact'])){
							showErr('企业联系人不能为空,请补充完资料后再申请',0,url_wap("user#register_two"));
						}elseif(empty($data['memberClassType'])){
							showErr('公司类型不能为空,请补充完资料后再申请',0,url_wap("user#register_two"));
						}
					}		
	...
	public function RegisterCat(){
		...
		if (empty($data)){
			...
			if (!empty($data['ips_acct_no'])){					
				...
				showErr('您未申请过资金托管帐户,请先申请资金托管'.$data['ips_acct_no'],0,str_replace("/wap", "", get_domain().$jump_url),0);
