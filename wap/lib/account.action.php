...
class accountModule{
	...
	{	
		...
		if($deal_type == 2)
		{
			$condition = " and d.type = 2 and ord.type in (0,2,3,7) ";//房产众筹订单
		}elseif($deal_type == 3)
		{
			$condition = " and d.type = 3 and ord.type in (0,2,3) ";//公益众筹订单
		}else{
			$condition = " and d.type = 0 and ord.type in (0,2,3) ";//产品众筹订单
		}
		...
		$order_list = $GLOBALS['db']->getAll("select ord.* from ".DB_PREFIX."deal_order as ord" .
				...
				" where ord.user_id = ".intval($GLOBALS['user_info']['id']).$condition." and d.is_delete=0 and d.is_effect=1 order by ord.create_time desc limit ".$limit);
		...
		$order_count = $GLOBALS['db']->getOne("select count(ord.id) from ".DB_PREFIX."deal_order as ord" .
				" left join ".DB_PREFIX."deal as d on d.id =ord.deal_id ".
				" where ord.user_id = ".intval($GLOBALS['user_info']['id']).$condition." and d.is_delete=0 and d.is_effect=1 ");
		$parameter_str="&deal_type=".$deal_type."";
...
//			}
	...
	public function project_public($project_deal_type=0,$project_act='project'){
	   ...
	   $condition_c=" type = ".$deal_type."";
		...
		$page = new Page($deal_count,$page_size,"&deal_type=".$deal_type."");   //初始化分页对象 		
		$p  =  $page->show();
		$GLOBALS['tmpl']->assign('pages',$p);
	...
	public function focus_public($focus_deal_type=0,$focus_act){
		...
		$page = new Page($deal_count,$page_size,$param_str);   //初始化分页对象 		
		$p  =  $page->show();
		$GLOBALS['tmpl']->assign('pages',$p);
	...
	public function mortgate_pay()
 	...
 	//股权参与列表
  	public function mine_investor_status(){
		...
		$GLOBALS['tmpl']->assign("page_title","股权投资的项目");
  	...
  	public function mine_investor_finance(){
		...
		$GLOBALS['tmpl']->assign("page_title","融资投资的项目");
  	...
  	public function mine_investor_status_public($investor_type,$investor_act){
		...
		if($type==0){
   		$investor_list=$GLOBALS['db']->getAll("select invest.*,d.end_time,d.pay_end_time,d.begin_time,d.name as deal_name ,d.image as deal_image,d.id as deal_id,d.is_success,d.stock_type" .
   				...
   				"where  (invest.type=0 or invest.type=5) and invest.user_id=$user_id and d.is_delete=0 and d.is_effect=1 ".$condition." order by invest.id desc limit $limit ");

  		$investor_list_num=$GLOBALS['db']->getOne("select count(invest.id)" .
   				" from  ".DB_PREFIX."investment_list as invest " .
   				"left join ".DB_PREFIX."deal as d on d.id=invest.deal_id " .
   				"where  (invest.type=0 or invest.type=5) and invest.user_id=$user_id and d.is_delete=0 and d.is_effect=1 ".$condition." ");
		}else{
			$investor_list=$GLOBALS['db']->getAll("select invest.*,d.end_time,d.pay_end_time,d.begin_time,d.name as deal_name ,d.image as deal_image,d.id as deal_id,d.is_success,d.stock_type" .
					" from  ".DB_PREFIX."investment_list as invest " .
					"left join ".DB_PREFIX."deal as d on d.id=invest.deal_id " .
					"where  invest.type=$type and invest.user_id=$user_id and d.is_delete=0 and d.is_effect=1 ".$condition." order by invest.id desc limit $limit ");
			
			$investor_list_num=$GLOBALS['db']->getOne("select count(invest.id)" .
					" from  ".DB_PREFIX."investment_list as invest " .
					"left join ".DB_PREFIX."deal as d on d.id=invest.deal_id " .
					"where  invest.type=$type and invest.user_id=$user_id and d.is_delete=0 and d.is_effect=1 ".$condition." ");
		}
 		...
 		if($type==0||$type==2||$type==1||$type==4){
  		...
  		$page = new Page($investor_list_num,$page_size,"&type=".$type."&deal_type=".$deal_type."");//初始化分页对象 		
		$p  =  $page->show();
		$GLOBALS['tmpl']->assign('pages',$p);
	...
	public function recommend_finance()
	{
  	...
  	public function recommend_public($recommend_deal_type=0,$recommend_act="recommend"){
		...
		$where=" and deal_type =".$deal_type." ";
  	
		$recommend_info=$GLOBALS['db']->getAll("SELECT * FROM ".DB_PREFIX."recommend WHERE user_id=".$user_id.$where." ORDER BY create_time DESC limit $limit");
		$recommend_count=$GLOBALS['db']->getOne("SELECT count(*) FROM ".DB_PREFIX."recommend WHERE user_id=".$user_id.$where);
  		
  		$page = new Page($recommend_count,$page_size,"&deal_type=".$deal_type."");   //初始化分页对象
  		$p  =  $page->show();
  		$GLOBALS['tmpl']->assign('pages',$p);
	...
	public function stock_transfer_insert()
	{
		...
		$data['invest_id']=intval($_REQUEST['id']);
