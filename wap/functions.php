...
function init_deal_page_wap($deal_info)
{
	...
	foreach ($deal_info['deal_item_list'] as $k=>$v){
			...
			$deal_info['deal_item_list'][$k]['cart_url']=url_wap("cart#index",array("id"=>$v['id'],'deal_id'=>$deal_info['id']));
			if($v['limit_user']){
...
function get_pre_wap(){
	if((ACT=="index")||
		...
		(ACT == "finance"&&ACT_2=="company_show")||
		(ACT == "finance"&&ACT_2=="company_finance")||
		(ACT == "finance"&&ACT_2=="company_update")||
		(ACT == "finance"&&ACT_2=="company_fans")||
		...
		(ACT == "score_mall"&&ACT_2=="index")||
		(ACT == "score_good_show"&&ACT_2=="index")
		)
		{	
			set_gopreview();
		}	
