		...
		$.ajax({
			...
			success:function(ajaxobj){
				...
				if(ajaxobj.status==2){
					...
					$.weeboxs.open(ajaxobj.html, {boxid:'enquiry_one',contentType:'text',showButton:false, showCancel:false, showOk:false,title:'项目投资',width:300,type:'wee'});
				}
				if(ajaxobj.status==4){
					//调取后续追加跟投页面
					$("body").find(".dialog-mask").first().remove();
					$("body").find(".weebox").first().remove();
					$.weeboxs.open(ajaxobj.html, {boxid:'enquiry_two',contentType:'text',showButton:false, showCancel:false, showOk:false,title:'项目追加投资',width:300,type:'wee'});
				}
