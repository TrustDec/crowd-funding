...
$(document).ready(function(){
	...
	try{
		...
		bind_drop_panel($("#zc_phone"),$("#zc_phone_drop").html(),"fade");
...
{
	$.weeboxs.open(str, {boxid:'fanwe_confirm_box',contentType:'text',showButton:true, showCancel:true, showOk:true,title:'提示',width:280,type:'wee',onok:func,onclose:funcls});
...
function bind_user_login()
{
	...
	$("#user_login_form").find("input[name='user_pwd']").bind("keydown",function(e){
		if(e.keyCode==9||e.keyCode==13)
		{
			$("#user_login_form").find("input[name='image_code']").focus();
			return false;
		}
	});
	$("#user_login_form").find("input[name='image_code']").bind("keydown",function(e){
		if(e.keyCode==13)
		{
			do_login_user();
		}
...
{
	if(empty($("#user_login_form").find("input[name='email']").val()))
	{
		$.showErr("请输入帐号",function(){
...
// 页面头像信息悬浮显示
function bind_drop_panel(o,html,effect)
{
	var timer;
	var drop_o = $(html);
	$(drop_o).hide();
	...
	if(effect == "fade"){
		$(o).hover(function(){
			...
			$(this).addClass("hover");
			$(".drop_box").fadeOut(300);
			clearTimeout(timer);
			$(drop_o).stop().fadeIn(300);
		},function(){		
...
{
	$.weeboxs.open(APP_ROOT+"/index.php?ctl=ajax&act=login", {boxid:'pop_user_login',contentType:'ajax',showButton:false, showCancel:false, showOk:false,title:'会员登录',width:736,type:'wee',onopen:function(){ui_checkbox();}});
}
...
function resetWindowBox(){
	...
	{	
		$("#J_wrap").css("height",(main_height-80)+"px");
	}
	resetTimeact = setTimeout(resetWindowBox,100);
}

// 单选复选
function bind_ui_check(){
...
function show_pop_region(){
	$.weeboxs.open(APP_ROOT+"/index.php?ctl=ajax&act=region", {boxid:'pop_region',contentType:'ajax',showButton:false, showCancel:false, showOk:false,title:'选择地区',width:720,type:'wee'});
}
