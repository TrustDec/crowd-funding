...
class messageModule extends BaseModule
{
	public function index()
	{
		...
		$user_id = intval($_REQUEST['dest_user_id']);
		$company_name = strim($_REQUEST['company_name']);
		$GLOBALS['tmpl']->assign("company_name",$company_name);
		if($user_id){
			$sql = "select max(id) as id,count(id) as total,dest_user_id from ".DB_PREFIX."user_message  where ((send_user_id = ".intval($GLOBALS['user_info']['id'])."  and receive_user_id = ".$user_id." and message_type = 'outbox') or (send_user_id = ".$user_id." and receive_user_id = ".intval($GLOBALS['user_info']['id'])."  and message_type = 'inbox')) group by dest_user_id order by create_time desc limit ".$limit;
			$sql_count = "select count(distinct(dest_user_id)) from ".DB_PREFIX."user_message where ((send_user_id = ".intval($GLOBALS['user_info']['id'])."  and receive_user_id = ".$user_id." and message_type = 'outbox') or (send_user_id = ".$user_id." and receive_user_id = ".intval($GLOBALS['user_info']['id'])."  and message_type = 'inbox'))";	
		}
		else{
		...
		$GLOBALS['tmpl']->assign("page_title","我的私信");
