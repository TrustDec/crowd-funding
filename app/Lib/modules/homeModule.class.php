...
// 		$deal_list['list']=$deal_invest_result['list'];
	...
	{	
		...
		{
  			app_redirect(url("home#deal_list",$param));
		}
		...
		foreach($cate_list as $k=>$v){
			if($v['pid'] == 0){
				...
				$temp_param['id'] = $v['id'];
				$cate_result[$k+1]['url'] = url("home#deal_list",$temp_param);
			}
		}
			...
			foreach($cate_list as $k=>$v)
			{
				if($v['pid'] == $pid){
					if($v['id'] > 0){
						...
						$temp_param['id'] = $v['id'];
						$child_cate_result[$v['id']]['url'] = url("home#deal_list",$temp_param);
						 if($id==$v['id']){
						 	$is_child = true;
						 }
       ...
       if($type ==0||$type ==3){
		...
		foreach($city_list as $k=>$v){
			...
			$city_list[$k]['url'] = url("home#deal_list",$temp_param);
		}
		...
		foreach($area_list as $k=>$v){
			...
			$area[strtoupper($v['py'][0])][$v['name']]=array('url'=> url("home#deal_list",$temp_param),'name'=>$v['name']);
			if($v['is_hot']){
				$hot_area[]=array('url'=> url("home#deal_list",$temp_param),'name'=>$v['name']);
			}
		...
		foreach($state_list as $k=>$v){
			$temp_param = $param;
			$temp_param['state'] = $k;
			$state_list[$k]['url'] = url("home#deal_list",$temp_param);
		}
		...
		$url_list=array(
 			'price_url'=>url("home#deal_list",$temp_param_price),
			'focus_url'=>url("home#deal_list",$temp_param_focus),
			'time_url'=>url("home#deal_list",$temp_param_time),
			'cp_url'=>url("home#deal_list",$temp_param_cp),
		);
 		//========
		if($price>0){
			if($price==1){
			...
			$url_list['price_url']=url("home#deal_list",$param_new);
			...
			$url_list['focus_url']=url("home#deal_list",$param_new);
 		}elseif($time>0){
			if($time ==1){
			...
			$url_list['time_url']=url("home#deal_list",$param_new);
		}elseif($cp>0){
			if($cp ==1){
				if($type==1){
			...
			$url_list['cp_url']=url("home#deal_list",$param_new);
		}else{
