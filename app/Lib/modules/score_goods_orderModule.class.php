...
class score_goods_orderModule extends BaseModule
{
	...
	public function del_order(){
		...
		if(!$order_info)
		{
			showErr("未找到订单",$ajax,url("score_goods_order#index"));
		}elseif($order_info['order_status'] !=0){
			showErr("该订单不能取消",$ajax,url("score_goods_order#index"));
		}
		
		
		if($GLOBALS['db']->query(" update ".DB_PREFIX."goods_order set order_status=4 where user_id=".$user_id." and id=".$id." and order_status=0"))
			showSuccess("取消成功",$ajax,url("score_goods_order#index"));
		else
			showErr("取消失败",$ajax,url("score_goods_order#index"));
