...
class projectModule extends BaseModule
{
	...
	public function selfless_index1()
	{
		...
		$GLOBALS['tmpl']->display("selfless_index1.html");
	}
	...
	public function del()
	{
		...
		if($GLOBALS['db']->affected_rows()>0)
		{
			...
			showSuccess("删除成功",$ajax,get_gopreview());
		}
		else
		{
			showErr("删除失败",$ajax);
	...
	{			
		...
		{		
			...
			showErr("",$ajax,url("project#add_item",array("id"=>$item['deal_id'])));
	...
	public function save_deal_item()
	{
		...
		if($id==0)
		{
			...
			if($result_id>0)
			{
				...
				showSuccess("保存成功",$ajax,url("project#add_item",array('id'=>$data['deal_id'])));
			}
			else
			{
				showErr("保存失败",$ajax);
...
///		}
		...
		$GLOBALS['msg']->manage_msg($GLOBALS['msg']::MSG_ZC_STATUS,'admin',array('deal_id'=>$id,'deal_status'=>$GLOBALS['msg']::CROW_EXAMINE));
	...
	public function choose()
	{
		...
		if($_REQUEST['company_id']){
			$GLOBALS['tmpl']->assign("company_id",$_REQUEST['company_id']);
		}
		$GLOBALS['tmpl']->assign("page_title","发起项目");
	...
	public function investor_one(){
		...
		$type=intval($_REQUEST['type']);
		if($type==4){
			$cate_list_str = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."deal_finance_cate order by sort asc");
		}else{
			$cate_list_str = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."deal_investor_cate order by sort asc");
		}
		...
		if($company_id){
			$company_info=$GLOBALS['db']->getRow("select * from ".DB_PREFIX."finance_company where id = ".$company_id." and status = 1  ");
		}
		if($type==4){
			
			if($company_info){
				$company_info['company_create_time']=to_date($company_info['company_create_time'],'Y-m-d');
				//公司图片简介
				$company_info['introduce_image']=unserialize($company_info['company_introduce_image']);
				if(sizeof($company_info['introduce_image'])>4){
					$company_info['introduce_image'] = array_slice($company_info['introduce_image'],0,4);
				}
				//最大融资轮数
				$invest_phase='';
				$project_invest_phase='';//通过审核项目 的最大融资轮数
				$company_investment_case='';//融资经历最大融资轮数
				$project_invest_phase=$GLOBALS['db']->getOne("select max(invest_phase) from ".DB_PREFIX."deal where user_id = ".$GLOBALS['user_info']['id']." and company_id =".$company_id." and is_effect=1 and is_delete=0 and type = 4");
				$company_investment_case=$GLOBALS['db']->getOne("select max(invest_phase) from ".DB_PREFIX."finance_company_investment_case where  company_id =".$company_id." and status =1");	
				$invest_phase=$project_invest_phase>$company_investment_case?$project_invest_phase:$company_investment_case;
				$GLOBALS['tmpl']->assign("invest_phase",$invest_phase);
				$GLOBALS['tmpl']->assign("type",$type);
			}
			else
			{
				showErr("该公司不存在或者审核未通过！");
			}	
		}
		//公司地址
		if($deal_item||$company_info){
			$region_pid = 0;
			$region_lv2 = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."region_conf where region_level = 2 order by py asc");  //二级地址
			foreach($region_lv2 as $k=>$v)
			{
				if($v['name'] == $deal_item['province'])
				{
					...
					$region_pid = $region_lv2[$k]['id'];
					break;
				}elseif($v['name']==$company_info['province']){
					$region_lv2[$k]['selected'] = 1;
					$region_pid = $region_lv2[$k]['id'];
					break;
				}
			...
			if($region_pid>0)
			{
				...
				foreach($region_lv3 as $k=>$v)
				{
					if($v['name'] == $deal_item['city'])
					{
						$region_lv3[$k]['selected'] = 1;
						break;
					}elseif($v['name'] == $company_info['city']){
						$region_lv3[$k]['selected'] = 1;
						break;
					}
				}
		...
		{
			if($type==4){
				$GLOBALS['tmpl']->assign("page_title",$deal_item['name']." - ".$company_info['company_name']);
			}else{
				$GLOBALS['tmpl']->assign("page_title",$deal_item['name']);
			}
		...
		else
		{
			...
			app_redirect(url("user#login"));
			if($type==4){
				$GLOBALS['tmpl']->assign("page_title","发起融资项目");	
			}else{
				$GLOBALS['tmpl']->assign("page_title","发起股权项目");
			}	
			$deal_image =  es_session::get("deal_image");
			$GLOBALS['tmpl']->assign("deal_image",$deal_image);
			$GLOBALS['tmpl']->display("investor_one.html");
...
*/		$data['business_name'] = strim($_REQUEST['business_name']);
		...
		$data['stock_type'] = intval($_REQUEST['stock_type']);
		...
		$data['exit_mechanism'] = strim($_REQUEST['exit_mechanism']);
		if($data['stock_type'] == 1){
			$data['share_fee_descripe'] = strim($_REQUEST['share_fee_descripe']);
			$data['annual_interest_rate'] = '';	
			$data['return_cycle'] = '';
			$data['return_descripe'] = '';
		}
		elseif($data['stock_type'] == 2)
		{
			$data['share_fee_descripe'] ='';
			$data['annual_interest_rate'] = floatval($_REQUEST['annual_interest_rate']);
			if($data['annual_interest_rate']=='')
			{
				showErr("预期年利率不能为空",$ajax,"");
			}
			$data['return_cycle'] = strim($_REQUEST['return_cycle']);
			$data['return_descripe'] = strim($_REQUEST['return_descripe']);
		}
		else{
			$data['share_fee_descripe'] = strim($_REQUEST['share_fee_descripe']);
			$data['annual_interest_rate'] = floatval($_REQUEST['annual_interest_rate']);
...
//		echo $_REQUEST['image'];exit;
		...
		if(intval($_REQUEST['deal_type'])==4){
			$data['invest_phase'] = intval($_REQUEST['invest_phase']);
			$data['company_id'] = intval($_REQUEST['company_id']);
			$data['type'] = 4;
		}else{
			$data['type']=1;
		}
		...
		else
		{
			...
			else
			{
				es_session::delete("deal_image");
				//插入图片
				insert_deal_images($images_array,$data_id);
				if($savenext==0)
				{
					showSuccess($data_id,$ajax,"");
				}
	...
	public function investor_six()
	{
		...
		//	print_r($attach_num);exit;
			$GLOBALS['tmpl']->assign("attach_num",$attach_num+1);
			$GLOBALS['tmpl']->assign("attach_list",$attach_list);
			$GLOBALS['tmpl']->assign("item",$item);
		}
		else
		{
			$GLOBALS['tmpl']->assign("attach_num",2);
		}
	...
	public function investor_edit()
	{
		...
		$GLOBALS['tmpl']->assign("audit_data_list",$audit_data_list);
		//公司信息
		$deal_item['company_name']=$GLOBALS['db']->getOne("select company_name from ".DB_PREFIX."finance_company where id =".$deal_item['company_id']);
