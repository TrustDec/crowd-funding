...
class score_good_showModule extends BaseModule
{
	...
	public function check_order()
	{
		...
		es_session::set("score_good_buy_info",$session_data);
		...
		$html=$GLOBALS['tmpl']->fetch("score/score_check_order.html");
		$data['status']=1;
		$data['html']=$html;
		ajax_return($data);
	}
	...
	{	
		...
		$score_good_buy_info=es_session::get("score_good_buy_info");
		$number=$score_good_buy_info['number'];
		$attr=$score_good_buy_info['attr'];
		...
		if(!$goods)
		{
			showErr("无效的商品",$ajax,url("index#index"));
			es_session::delete("score_good_buy_info");
		}
			
		if($score_good_buy_info['user_id'] != $user_id)
		{
			showErr("请重新选择下单",$ajax,url("score_good_show#index",array('id'=>$id)));
			es_session::delete("score_good_buy_info");
		}
		
		if($score_good_buy_info['id'] != $id)
		{
			showErr("请重新选择下单",$ajax,url("score_good_show#index",array('id'=>$id)));
			es_session::delete("score_good_buy_info");
		}
		...
		{	
			es_session::delete("score_good_buy_info");
			showErr($stock_return['info'],$ajax,url("score_good_show#index",array('id'=>$id)));
		}
		//写入属性
		...
		{
			es_session::delete("score_good_buy_info");
			showErr("您的积分不够",$ajax,url("score_good_show#index",array('id'=>$id)));
		}
		...
		{	es_session::delete("score_good_buy_info");
			...
			if(!$score_status)
			{
				...
				showErr("兑换失败，订单无效",$ajax,url("score_good_show#index",array('id'=>$id)));
			}else
			{
			...
			if($stock_return['status'] ==0)
			{
				...
				$GLOBALS['db']->query("update ".DB_PREFIX."goods_order set order_status=".$order_data['order_status']." where id=".intval($order_id));
				showErr("兑换失败，库存不够，积分退回",$ajax,url("score_good_show#index",array('id'=>$id)));
			}else
			{
				$order_data['order_status']=1;//兑换成功
		...
		{
			showErr("兑换失败",$ajax,url("score_good_show#index",array('id'=>$id)));
		}
		
		$ajax_return['status']=1;
		$ajax_return['info']="兑换成功";
		showSuccess("兑换成功",$ajax,url("score_goods_order#index"));
	}
}
?>