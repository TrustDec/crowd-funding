...
class licaiModule extends BaseModule
{
	...
	public function index()
	{
		...
		//获取普通推荐
		$rectype_list = get_licai_list("status=1 and is_recommend = 1","sort DESC,id DESC",5);
		...
		$image_list = load_auto_cache("index_image");
 		$GLOBALS['tmpl']->assign("image_list",$image_list[3]); 
	...
	public function deals(){
		...
		//为客户创造收益
		//$user_income = float($GLOBALS['db']->getOneCached("select sum(earn_money) from ".DB_PREFIX."user_log WHERE `type`=9 "));
	...
	public function deal(){
		...
		if(!$licai || $licai['status'] != 0){
			$licai_interest = get_licai_interest_yeb($licai["id"],to_date(TIME_UTC,"Y-m-d"),$licai["end_interest_date"]);
			$licai['average_income_rate'] = $licai_interest['avg_interest_rate'];
		}	
		$GLOBALS['tmpl']->assign("licai",$licai);
		$min_interest_rate=0;
		$min_interest_rate=0;
		...
		//为客户创造收益
		//$user_income = float($GLOBALS['db']->getOneCached("select sum(money) from ".DB_PREFIX."user_log WHERE `type`=9 "));
		$user_income = floatval($GLOBALS['db']->getOneCached("select sum(earn_money) from ".DB_PREFIX."licai_redempte"));
		$GLOBALS['tmpl']->assign("user_income",$user_income);
		
		$GLOBALS['tmpl']->assign('page_title',$licai['name']);
	...
	public function uc_record_lc(){
		...
		$list = $GLOBALS["db"]->getAll("select lco.*,lc.type from ".DB_PREFIX."licai_order lco 
		left join ".DB_PREFIX."licai lc on lc.id = lco.licai_id 
		...
		foreach($list as $k=>$v)
		{
			...
			if($v['type']>0){
				...
				$list[$k]["interest_rate_format"] = number_format($v["interest_rate"],4)."%";;
			}else{
				$licai_interest = get_licai_interest_yeb($v['licai_id'],$v["begin_interest_date"],$v["end_interest_date"]);
				$list[$k]["interest_rate_format"] =  number_format($licai_interest['avg_interest_rate'],4)."%";
			}
	...
	public function uc_expire_lc(){
		...
		foreach($list as $k=>$v)
		{
			...
			if($v["licai_type"] > 0)
			{
				$list[$k]["before_rate_format"] = number_format($v["before_rate"],4)."%";
				$list[$k]["interest_rate_format"] = number_format($v["interest_rate"],4)."%";
			}
			else
			{
				$licai_interest = get_licai_interest_yeb($v['licai_id'],$v["begin_interest_date"],$v["end_interest_date"]);
				$list[$k]["rate_format"] = number_format($licai_interest["avg_interest_rate"],4)."%";
			}
			if($v["licai_type"] == 0)
			{
	...
	public function uc_redeem_lc(){
		...
		,lc.type as licai_type,lcr.type from ".DB_PREFIX."licai_redempte lcr 
		left join ".DB_PREFIX."licai_order lco on lco.id = lcr.order_id 
		left join ".DB_PREFIX."licai lc on lco.licai_id = lc.id 
		...
		foreach($list as $k=>$v)
		{
			...
			if($v["licai_type"] > 0)
			{
				...
				if($v["type"] == 0 )
				{
					...
					$list[$k]["rata_format"] = number_format($v["rate"],4)."%";
				}
				elseif($v["type"] == 1)
				{
				...
				elseif($v["type"] == 3)
				{
					$list[$k]["rate"] = $v["rate"] =  $licai_interest["before_rate"];
					$list[$k]["rata_format"] = number_format($v["rate"],4)."%";
				}
				else
				{
	...
	public function uc_buyed_lc(){
		...
		foreach($list as $k=>$v)
		{
			...
			else
			{
				...
				$list[$k]["interest_rate_format"] = number_format($licai_intereset["avg_interest_rate"],4)."%";
			}
			switch($v["status"])
			{
				case 0 :
	...
	public function uc_buyed_deal_lc(){
		...
		sum(case when lcr.status = 1 then (lcr.money) else 0 end) as licai_all_redempte,
		...
		foreach($list as $k=>$v)
		{
			...
			elseif($v["type"] == 3)
			{
				if($v["type"]){
					...
					$list[$k]["type_format"] = "预热期正常赎回";//预热期正常赎回
				}
...
//		}
	...
	public function uc_redeem(){
		...
		left join ".DB_PREFIX."licai_redempte lcr on lcr.order_id = lco.id where lco.id=".$id);
		...
		if($licai['type'] > 0){
			...
			if(to_timespan($vo["end_interest_date"])>TIME_UTC){
				$licai_interest = get_licai_interest_yeb($vo["licai_id"],$vo["begin_interest_date"],to_date(TIME_UTC,"Y-m-d"));
				$licai['average_income_rate'] = $licai_interest["avg_interest_rate"];
			}
		...
		if($vo['before_interest_date']<$now&&$vo['before_interest_enddate']>$now){
			...
			if($day<=0){
				$day=0;
				if($vo['begin_interest_date'] !=$vo['before_interest_enddate']){
					$vo["licai_status"] = 1;
				}else{
					$vo["licai_status"] = 3;
				}
	...
	function uc_redeem_add()
	{
		...
		$paypassword = strim($_REQUEST["password"]);
	...
	public function set_status()
	{
		...
		$licai_order = $GLOBALS["db"]->getRow("select lco.*,u.user_name,lc.pay_type from ".DB_PREFIX."licai_order lco 
		left join ".DB_PREFIX."licai lc on lco.licai_id = lc.id 
		left join ".DB_PREFIX."user u on u.id = lco.user_id 
		where lco.id=".$id." and lc.user_id = ".$GLOBALS["user_info"]["id"]);

		if(intval($_REQUEST["pay_type"])){
		...
		if($result['lack']){
			$result["jump"] = url("account#money_inchange");
		}else{
			$result["jump"] = url("licai#uc_expire_lc");
		}
		//修改状态
	...
	//冻结金明细
	public function mortgage_money()
     {
