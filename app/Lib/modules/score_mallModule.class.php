...
class score_mallModule extends BaseModule
{
	...
	{	
		...
		$cate_list[0]['id']=0;
		$cate_list[0]['name']="不限";
		$cate_list[0]['url'] = url("score_mall#index",$param_cate);
		foreach($cates_blist as $k=>$v)
		{
			$cate_list[$k]=$v;
			$param_cate['cates']=$v['id'];
			$cate_list[$k]['url']=url("score_mall#index",$param_cate);
			if($cate_list[$k]['sub_list'])
			{	
				foreach($cate_list[$k]['sub_list'] as $kk=>$vv)
				{
					$param_cate['cates']=$vv['id'];
					$cate_list[$k]['sub_list'][$kk]['url']=url("score_mall#index",$param_cate);
				}
			}
		}
		//小分类
		...
		foreach($integral_url as $k=>$v){
			...
			$integral_url[$k]['url'] = url("score_mall#index",$param_integral);
		}
		...
		foreach($sort_info as $k=>$v){
			...
			$param_sort['sort'] = $k;
			$sort_info[$k]['url'] = url("score_mall#index",$param_sort);
		}
		...
		if($page==0)
			$page = 1;
		$page_size=SCORE_GOODS_STEP_SIZE;
		...
		foreach($GLOBALS['nav_list'] as $k=>$v){
			if($v['u_module']=='score_mall'&&$v['u_action']=='index'&&$v['u_param']==''){
					$page_title=$v['name'];
				}		
 		}
		$page_title=$page_title?$page_title:'积分商城';
